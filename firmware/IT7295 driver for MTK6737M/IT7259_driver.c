		...
		unsigned char readDataBuffer[], unsigned short readDataLength) 
{
	...
	//printk("====in i2cReadFromIT7259 function===\n");
		...
		unsigned char const dataBuffer[], unsigned short dataLength)
{
	...
	//printk("====in i2cWriteToIT7259 function===\n");
...
bool IT7259_SendCommand( unsigned char writeBuffer[], unsigned short writeLength,
	...
	pr_err("====Response--[%x][%x][%x][%x][%x][%x]=\n", readDataBuffer[0], readDataBuffer[1],
			readDataBuffer[2], readDataBuffer[3], readDataBuffer[4], readDataBuffer[5]);
	

	return true;
}
void gfnIT7257_write_signature(void)
{
	unsigned char DATABuffer[2]; 	

		DATABuffer[0] = 0x59;
    DATABuffer[1] = 0x72;
    gfnIT7259_DirectEraseFlash(0xD7,0xFC00);
    gfnIT7259_DirectWriteFlash(0xFC00,2,DATABuffer);
			...
			//printk("-----------------------------------------------pucPoint[0] & 0xF0---------------------.\n");
		    	...
		    	if (  pucPoint[0] & 0x40  && pucPoint[0] & 0x01){
		    	    ...
		    	    if ( pucPoint[2] ){
						...
						input_report_key(tpd->dev, tpd_it7259_keys[(pucPoint[1]-1)],1);//ysong
						input_sync(tpd->dev);
						key_temp = pucPoint[1]-1;
		       		}
					else{
						//tpd_button(tpd_keys_dim_local[pucPoint[1]-1][0], tpd_keys_dim_local[pucPoint[1]-1][1], 1); 
						input_report_key(tpd->dev, tpd_it7259_keys[key_temp],0);
						input_sync(tpd->dev);
					}
		    	}else
		    #endif
